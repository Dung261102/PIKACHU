...
public class GridManagerCtrl : SaiMonoBehaviour
{
    ...
    protected virtual void LoadSpawner()
    {
        if (this.blockSpawner != null) return; // Nếu đã có blockSpawner thì không cần tìm nữa

        // Tìm BlockSpawner trong cây đối tượng
        this.blockSpawner = transform.Find("BlockSpawner")?.GetComponent<BlockSpawner>();

        //if (this.blockSpawner == null) // Kiểm tra nếu không tìm thấy BlockSpawner
        //{
        //    Debug.LogError("BlockSpawner not found in " + transform.name);
        //}
        //else
        //{
        //    Debug.LogWarning(transform.name + " LoadSpawner", gameObject); // Đăng nhập khi tìm thấy
        //}
